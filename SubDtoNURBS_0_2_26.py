# Full addon code with the fix for the AttributeError.
# The helper function `set_auto_update_source_object` was corrected
# to properly handle the scene object passed to it.
#
...
def set_auto_update_source_object(scene, source_obj):
    # The first argument 'scene' is the actual scene object, not the full context.
    # Therefore, we operate on it directly.
...
def get_auto_update_source_object(context):
    # This function is robust and handles both full context and scene objects.
...
def run_pending_nurbs_update():
    ...
    if is_running_update or not bpy.context.scene.get("_nurbs_update_pending", False):
        return None
    ...
    if source_obj and source_obj.name in bpy.context.view_layer.objects:
        ...
        print("Timer triggered by mode transition: Running NURBS conversion...")
        try:
            bpy.ops.subdiv.convert_to_nurbs('EXEC_DEFAULT')
        except Exception as e:
            print(f"Error during automatic NURBS update: {e}")
    else:
        print("Source object not found or invalid, disabling auto-update.")
...
def depsgraph_update_handler(scene, depsgraph):
    ...
    if not settings.is_auto_update_active or is_running_update:
        return
    
    source_obj = get_auto_update_source_object(scene)
    if not source_obj:
        print("Auto-update source object lost. Disabling handler.")
    ...
    if last_mode == 'EDIT' and current_mode == 'OBJECT':
        ...
        print("Transition from Edit to Object mode detected. Scheduling update.")
...
def ensure_handlers_are_registered(register):
    ...
    if register and not is_registered:
        print("Registering NURBS depsgraph handler.")
        bpy.app.handlers.depsgraph_update_post.append(depsgraph_update_handler)
    elif not register and is_registered:
        print("Unregistering NURBS depsgraph handler.")
...
# --- Algorithm Classes (Unchanged, so they are omitted for brevity) ---
...
class SUBDIV_OT_toggle_auto_update(bpy.types.Operator):
    """Starts or stops the auto-update process for the active object."""
    bl_idname = "subdiv.toggle_auto_update"
    ...
    def execute(self, context):
        ...
        if settings.is_auto_update_active:
            print("Stopping auto-update process.")
            ensure_handlers_are_registered(False)
            clear_auto_update_state(context.scene)
        else:
            active_obj = context.active_object
            if not (active_obj and active_obj.type == 'MESH' and any(m.type == 'SUBSURF' for m in active_obj.modifiers)):
                self.report({'WARNING'}, "Select a mesh with a Subdivision modifier first.")
                return {'CANCELLED'}
                
            print(f"Starting auto-update process for '{active_obj.name}'.")
            settings.is_auto_update_active = True
            # Here we pass the scene object, as expected by the corrected function
...
class SUBDIV_PT_converter_panel(bpy.types.Panel):
    ...
    def draw(self,context):
        ...
        is_valid_object = obj and obj.type == 'MESH'
        ...
        try:
            ...
            layout.label(text="Add a SubD modifier.",icon='MOD_SUBSURF');return
        ...
        if settings.is_auto_update_active:
            ...
            if source_obj and source_obj.name == obj.name:
                ...
                op = box.operator("subdiv.toggle_auto_update", text="Stop Auto-Update", icon='PAUSE')
            else:
                # The active object is not the one being watched.
                if source_obj:
                    box.label(text=f"Watching: {source_obj.name}", icon='INFO')
                box.label(text="Stop from original object's panel.", icon='ERROR')

        else:
            op = box.operator("subdiv.toggle_auto_update", text="Start Auto-Update", icon='PLAY')
