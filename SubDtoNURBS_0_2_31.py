# --- START OF FILE SubDtoNURBS_0_8_3.py ---

bl_info = {
    "name": "SubD to NURBS (v0.2.31) non-freezing UI and a custom progress cursor",
    "author": "oobma, Gemini (Final Architect)",
    "version": (0, 2, 31),
    "blender": (4, 1, 0),
    "location": "View3D > Sidebar > SubDtoNURBS | Edit > Preferences > Add-ons",
    "description": "Convert subdivision surfaces to NURBS export patches to IGES.",
    "category": "Object",
...
# --- State Management for Auto-Update ---
...
def run_pending_nurbs_update():
    ...
    if source_obj and source_obj.name in bpy.context.view_layer.objects:
        ...
        try:
            bpy.ops.subdiv.convert_to_nurbs('INVOKE_DEFAULT')
...
class EdgeCentricStitcher:
    ...
    def _enforce_G0(self):
        for e in self.bm.edges:
            if e.is_boundary or len(self.map.get(e.index, [])) == 0: continue
            edge_map = self.map.get(e.index, [])
            if len(edge_map) == 2:
                (fa_idx, sa), (fb_idx, sb) = edge_map; g0_boundary = None
                ...
                (fa_idx, sa) = edge_map[0]
                g0_boundary = self._get_b(self.initial[fa_idx], sa)
                cos, coe = Vector(self.verts[e.verts[0].index]), Vector(self.verts[e.verts[1].index])
                if (g0_boundary[0] - cos).length_squared > (g0_boundary[0] - coe).length_squared: cos, coe = coe, cos
                g0_boundary[0], g0_boundary[3] = cos, coe
                self._set_b(self.final[fa_idx], sa, g0_boundary)
    def _build_G1_system(self):
        v_map = {(f,r,c):i for i, (f,r,c) in enumerate((fi,r,c) for fi in self.initial for r in range(1,3) for c in range(1,3))}
        if not v_map: return None, None, None
        rows, b_vectors = [], []
        w_boundary = 1.0
        for e in self.bm.edges:
            edge_map = self.map.get(e.index, [])
            if not edge_map: continue
            if len(edge_map) == 2 and self.crease_data[e.index] < 0.99:
                (fa,sa),(fb,sb)=edge_map
                ca,cb,bd=self._get_i(None, sa, get_indices=True),self._get_i(None, sb, reverse=True, get_indices=True),self._get_bi(None, sa, get_indices=True)
                ...
                (f_idx, side) = edge_map[0]
                p0_indices = self._get_b(None, side, get_indices=True)
                p1_indices = self._get_i(None, side, get_indices=True)
                p2_indices = self._get_i2(None, side, get_indices=True)
                for i in range(1, 3):
                    p0_r, p0_c = p0_indices[i]
                    p0_val = self.final[f_idx][p0_r][p0_c]
                    p1_r, p1_c = p1_indices[i]
                    p2_r, p2_c = p2_indices[i]
                    row = []; b_val = w_boundary * p0_val
                    p1_tuple = (f_idx, p1_r, p1_c)
                    if p1_tuple in v_map: row.append((v_map[p1_tuple], 2.0 * w_boundary))
                    else: b_val -= 2.0 * w_boundary * self.final[f_idx][p1_r][p1_c]
                    p2_tuple = (f_idx, p2_r, p2_c)
                    if p2_tuple in v_map: row.append((v_map[p2_tuple], -1.0 * w_boundary))
                    else: b_val -= -1.0 * w_boundary * self.final[f_idx][p2_r][p2_c]
                    if row: rows.append(row); b_vectors.append(b_val)
        ...
        return v_map, rows, b_vectors
    def _solve_G1_numpy(self):
        v_map, rows, b_vectors = self._build_G1_system()
        if not v_map: return
        ...
        for i, row in enumerate(rows):
            for i1, v1 in row:
                ...
                for i2, v2 in row:
                    A[i1, i2] += v1 * v2
        A_csr = A.tocsr()
        sol_x, _ = cg(A_csr, Atb[:,0], rtol=1e-5)
        sol_y, _ = cg(A_csr, Atb[:,1], rtol=1e-5)
        sol_z, _ = cg(A_csr, Atb[:,2], rtol=1e-5)
        for(f,r,c),i in v_map.items():self.final[f][r][c]=Vector((sol_x[i],sol_y[i],sol_z[i]))
    def _solve_G1(self):
        if SCIPY_AVAILABLE: self._solve_G1_scipy()
        else: self._solve_G1_numpy()
    def _get_b(self, p, s, reverse=False, get_indices=False):
        if get_indices:
    ...
    def _get_bi(self, p, s, reverse=False, get_indices=False):
        return self._get_b(p, s, reverse=reverse, get_indices=True)
    def _get_i(self, p, s, reverse=False, get_indices=False):
        if get_indices:
            ...
            return r[::-1] if reverse else r
    def _get_i2(self, p, s, reverse=False, get_indices=False):
        if get_indices:
            if s=='U0': r=[(2,i) for i in range(4)]
            elif s=='U1': r=[(1,3-i) for i in range(4)]
            elif s=='V0': r=[(3-i,2) for i in range(4)]
            else: r=[(i,1) for i in range(4)]
            return r[::-1] if reverse else r
...
class LimitSurfaceFitter:
    ...
    def run_conversion_generator(self):
        ...
        yield "Pre-fetching geometry data..."
        ...
        yield "Phase 1: Initial patch fitting..."
        self.oriented_fit_cache={};self.ruled_map={};self.fallback_face_indices=set()
        quad_faces = [f for f in bm_orig.faces if len(f.verts) == 4]
        total_faces = len(quad_faces)
        for i, f in enumerate(quad_faces):
            if (i % 10) == 0:
                yield f"Fitting patch {i}/{total_faces}"
        ...
        if not self.oriented_fit_cache:
            self.report({'ERROR'},"Initial adjustment failure.");bm_orig.free();bm_eval.free()
            yield "ERROR"
            return
        yield "Phase 2: Global stitching..."
...
class SUBDIV_OT_convert_to_nurbs(bpy.types.Operator):
    bl_idname="subdiv.convert_to_nurbs"
    bl_label="Convert SubD to Patches"
    bl_options={'REGISTER','UNDO'}
    
    _timer = None
    fitter = None
    fitter_generator = None
    _draw_handler = None
    mouse_pos = (0, 0)
    progress = 0.0
    ...
    # --- NEW: Robust redraw function ---
    def _force_ui_update(self, context):
        """Forces a redraw of all 3D Viewports. Necessary for headless contexts."""
    ...
    def draw_callback(self):
        ...
        text_width, text_height = blf.dimensions(font_id, progress_text)
        box_padding = int(5 * ui_scale)
        box_width = text_width + 2 * box_padding
        box_height = text_height + 2 * box_padding
        box_x = self.mouse_pos[0] - box_width / 2
        box_y = self.mouse_pos[1] - box_height / 2
        gpu.state.blend_set('ALPHA')
        draw_rectangle(box_x, box_y, box_width, box_height, (0.1, 0.1, 0.1, 0.7))
        text_y_offset = int(2 * ui_scale)
        blf.position(font_id, box_x + box_padding, box_y + box_padding + text_y_offset, 0)
    ...
    def modal(self, context, event):
        ...
        if event.type == 'MOUSEMOVE':
            ...
            # We don't need a redraw here, the timer handles it.

        if event.type == 'TIMER':
            try:
                # --- FIX: Use the robust redraw function ---
                self._force_ui_update(context)

                progress_message = next(self.fitter_generator)
                if "Fitting patch" in progress_message:
                    try:
                        parts = progress_message.split(' ')[2].split('/')
                        current, total = int(parts[0]), int(parts[1])
                        if total > 0: self.progress = (current / total) * 100.0
                    except (ValueError, IndexError): pass
                elif "Phase 2" in progress_message: self.progress = 95.0
                elif "Phase 3" in progress_message: self.progress = 98.0
                context.workspace.status_text_set(f"SubD to NURBS: {progress_message}")
                if "FINISHED" in progress_message or "ERROR" in progress_message:
                    self.finish(context)
                    return {'FINISHED'}
                ...
                self.report({'ERROR'}, f"Unexpected error during modal operation: {e}")
    ...
    def invoke(self, context, event):
        ...
        self.fitter = LimitSurfaceFitter(context, obj, settings)
        self.fitter.report = self.report
        self.fitter_generator = self.fitter.run_conversion_generator()
        
        self._draw_handler = bpy.types.SpaceView3D.draw_handler_add(self.draw_callback, (), 'WINDOW', 'POST_PIXEL')
        
        # Check if the event is a valid mouse event before accessing coordinates
        if event and hasattr(event, 'mouse_x'):
            self.mouse_pos = (event.mouse_x, event.mouse_y)
        else:
            # Fallback for headless context: place cursor in the center of the window
            if context.window:
                self.mouse_pos = (context.window.width / 2, context.window.height / 2)
            else:
                self.mouse_pos = (0,0) # Should not happen in practice
    ...
    def finish(self, context):
        ...
        context.workspace.status_text_set("SubD to NURBS: Conversion complete.")
...
class EXPORT_OT_iges(bpy.types.Operator):
    ...
    # --- FIX: Restore the correct invoke method ---
    def invoke(self, context, event):
        blend_filename = Path(bpy.data.filepath).stem
        if not blend_filename:
            self.filepath = "Untitled.igs"
        else:
            self.filepath = f"{blend_filename}.igs"
    ...
    def _extract_patch_data(self, obj):
        ...
        if obj.type == 'SURFACE' and obj.data.splines and obj.data.splines[0].type == 'NURBS':
            ...
            num_u, num_v, order_u, order_v = spline.point_count_u, spline.point_count_v, spline.order_u, spline.order_v
    ...
    def _process_patch(self, data, start_de_num, start_pd_num):
        ...
        knots_u, knots_v = self._generate_clamped_knot_vector(data['order_u'], data['num_u']), self._generate_clamped_knot_vector(data['order_v'], data['num_v'])
        params = ["128", data['num_u'] - 1, data['num_v'] - 1, degree_u, degree_v, "0", "0", "1", "0", "0"]
        params.extend(knots_u); params.extend(knots_v); params.extend(data['weights']); params.extend(data['cps']); params.extend([0.0, 1.0, 0.0, 1.0])
        ...
        final_line_str = current_line[:-1] + ";"; content = f"{final_line_str.ljust(64)}{de_ptr_val:>8}"
        pd_lines.append(self._format_line('P', content, current_pd_num))
...
class SUBDIV_OT_install_dependencies(bpy.types.Operator):
    ...
    bl_description = "Downloads and installs SciPy into Blender's user scripts folder."
    def invoke(self, context, event):
