bl_info = {
    "name": "Limit Surface to NURBS (v49.7 - Final Architecture)",
...
class LimitSurfaceFitter:
    ...
    def run_conversion(self):
        ...
        if self.settings.generate_ruled_surfaces:
             self.ruled_map = {f.index: True for f in bm_orig.faces if self.check_if_ruled_face(f, crease_layer)}
        planar_map = {idx: is_patch_truly_flat(data['cps'], self.settings.planar_tolerance) for idx, data in self.oriented_fit_cache.items() if idx not in self.ruled_map}
        
        print("Phase 2: Global sewing...")
        stitcher=EdgeCentricStitcher(bm_orig,self.oriented_fit_cache,self.obj,self.settings, planar_map, self.ruled_map, crease_layer)
        final_patches=stitcher.run()
        if self.settings.generate_ruled_surfaces or self.settings.force_planar_faces:
            print("Phase 2.5: Correction for design intent...")
            if self.settings.generate_ruled_surfaces: final_patches=self._enforce_ruled_patches(final_patches)
            if self.settings.force_planar_faces: final_patches=self.get_planarized_patches(final_patches)
    ...
    def _enforce_ruled_patches(self, stitched_patches):
        print("   - Aplicando post-proceso de superficies regladas...")
        ruled_faces_count = 0
        for face_idx, control_points in stitched_patches.items():
            if not self.ruled_map.get(face_idx, False): continue
            ruled_faces_count += 1
            orientation = self.oriented_fit_cache[face_idx]['orientation']
            # Simplified: if it was marked as ruled, one of the two directions is correct.
            # It is assumed that the crease check has already validated it.
            if orientation == 0: # U-rows
                row0, row3 = [p.copy() for p in control_points[0]], [p.copy() for p in control_points[3]]
                control_points[1] = [p0.lerp(p3, 1/3) for p0, p3 in zip(row0, row3)]
                control_points[2] = [p0.lerp(p3, 2/3) for p0, p3 in zip(row0, row3)]
            else: # V-rows
                col0=[control_points[i][0].copy() for i in range(4)]; col3=[control_points[i][3].copy() for i in range(4)]
                col1=[p0.lerp(p3, 1/3) for p0, p3 in zip(col0, col3)]; col2=[p0.lerp(p3, 2/3) for p0, p3 in zip(col0, col3)]
                for i in range(4): control_points[i][1]=col1[i]; control_points[i][2]=col2[i]
        print(f"     - Ruled correction applied to {ruled_faces_count} patches.")
        return stitched_patches
