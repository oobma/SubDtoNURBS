bl_info = {
    "name": "SubD to NURBS (v0.2.28) (FIXED)",
...
class EXPORT_OT_iges(bpy.types.Operator):
    ...
    def execute(self, context):
        # --- NEW: Filepath Extension Check ---
        # Ensure the output file has a valid IGES extension. This is a crucial UX improvement.
        if not self.filepath.lower().endswith(('.igs', '.iges')):
            self.filepath += ".igs" # Append the default extension if missing
        ...
        # --- Section Buffers and Line Counters ---
    ...
    def invoke(self, context, event):
        # Set a default filename to appear in the file dialog
        # This is another UX improvement.
    ...
    # --- Helper Functions for IGES Formatting (Unchanged from previous fix) ---

    def _format_line(self, section_char, content, line_num):
        # Centralized function for formatting any line to ensure consistency.
        return f"{content.ljust(72)}{section_char}{line_num:>7}\n"

    def _generate_start_section(self, buffer):
        # Generates the S (Start) section.
        buffer.append(self._format_line('S', "Exported from Blender using SubD to NURBS Addon v0.2.28", 1))

    def _generate_global_section(self, buffer):
        # Generates the G (Global) section with metadata.
    ...
    def _generate_clamped_knot_vector(self, order, num_points):
        # Generates a standard clamped knot vector for a single patch.
        knots = [0.0] * order
        knots.extend([1.0] * order)
        return knots

    def _extract_patch_data(self, obj):
        # Extracts surface data from either a Blender NURBS or a PsychoPatch MESH.
        ...
        if obj.type == 'SURFACE' and obj.data.splines and obj.data.splines[0].type == 'NURBS':
            ...
            cps_flat, weights_flat = [], []
            for u_idx in range(num_u):
                for v_idx in range(num_v):
                    blender_idx = u_idx * num_v + v_idx
                    pt = spline.points[blender_idx]
                    coord = (matrix @ pt.co.to_3d()) * 1000.0
                    cps_flat.extend([coord.x, coord.y, coord.z])
                    weights_flat.append(pt.weight)
            
            return {
                "name": obj.name, "num_u": num_u, "num_v": num_v,
                "order_u": order_u, "order_v": order_v,
                "cps": cps_flat, "weights": weights_flat
            }
            ...
            for vert in obj.data.vertices:
                coord = (matrix @ vert.co) * 1000.0
                cps_flat.extend([coord.x, coord.y, coord.z])
            
            return {
                "name": obj.name, "num_u": num_u, "num_v": num_v,
                "order_u": order_u, "order_v": order_v,
                "cps": cps_flat, "weights": [1.0] * 16
            }
            
        return None

    def _process_patch(self, data, start_de_num, start_pd_num):
        # Generates D and P sections for a single patch with strict formatting.
        de_lines, pd_lines = [], []
        de_ptr_val = start_de_num

        # --- 1. Assemble Parameter List ---
        ...
        params = [
            "128", data['num_u'] - 1, data['num_v'] - 1, degree_u, degree_v,
            "0", "0", "1", "0", "0"
        ]
        params.extend(knots_u)
        params.extend(knots_v)
        params.extend(data['weights'])
        params.extend(data['cps'])
        params.extend([0.0, 1.0, 0.0, 1.0])

        # --- 2. Generate Parameter Data (P) Section Lines ---
        ...
        # --- 3. Generate Directory Entry (D) Section Lines ---
        num_param_lines = len(pd_lines)
        
        line1_data = (f"{128:>8}" f"{start_pd_num:>8}" f"{0:>8}" f"{0:>8}"
                      f"{0:>8}" f"{0:>8}" f"{0:>8}" f"{0:>8}" f"{'00000001':>8}")
        de_lines.append(self._format_line('D', line1_data, start_de_num))
        
        line2_data = (f"{128:>8}" f"{0:>8}" f"{0:>8}" f"{num_param_lines:>8}"
                      f"{1:>8}" f"{0:>8}" f"{0:>8}" f"{data['name'].ljust(8)[:8]:<8}" f"{0:>8}")
        de_lines.append(self._format_line('D', line2_data, start_de_num + 1))
        
        # --- 4. Return generated lines and updated counters ---
